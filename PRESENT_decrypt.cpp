...
// Main function to read ciphertext from file and write decrypted plaintext to another file
int main() {
    ...
    std::ifstream inputFile("output_present_encrypt.txt");
    std::string ciphertext;
    if (inputFile.is_open()) {
        std::getline(inputFile, ciphertext);
        inputFile.close();
    } else {
        std::cerr << "Error: Unable to open 'output_present_encrypt.txt'\n";
        return 1;
    }
    ...
    // Save the decrypted plaintext to output_present_decrypt.txt
    std::ofstream outputFile("output_present_decrypt.txt");
    if (outputFile.is_open()) {
        outputFile << decryptedText;
        outputFile.close();
    } else {
        std::cerr << "Error: Unable to open 'output_present_decrypt.txt'\n";
        return 1;
    }

    // Show the decrypted plaintext on screen
