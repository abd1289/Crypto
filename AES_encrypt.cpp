...
int main() {
    ...
    int original_len = message.length();
    int padded_len = ((original_len / 16) + 1) * 16;

    unsigned char *padded_msg = new unsigned char[padded_len];
    for (int i = 0; i < padded_len; i++) {
        padded_msg[i] = (i < original_len) ? message[i] : 0;
    }
    ...
    // Write encrypted text to "output_aes_encrypt.txt"
    ofstream outputFile("output_aes_encrypt.txt");
    if (!outputFile) {
    ...
    // Show encrypted text on console
    cout << "Encrypted Message:" << endl;
    for (int i = 0; i < padded_len; i++) {
        print_hex(padded_msg[i]);
        cout << " ";
        str1 = byte_to_hex(padded_msg[i]);
        outputFile << str1;
    }
    cout << endl;
