...
int main() {
    ...
    std::cout << "Enter the plaintext (8 characters): ";
    std::getline(std::cin, inputText);

    if (inputText.length() != 8) {
        std::cerr << "Invalid input length. Please enter exactly 8 characters.\n";
        return 1;
    }

    uint32_t left = 0, right = 0;
    for (int i = 0; i < 4; ++i) {
        left |= inputText[i] << (24 - i * 8);
        right |= inputText[4 + i] << (24 - i * 8);
    }

    std::vector<uint8_t> key = { /* 16-byte key */ };
    ...
    std::ofstream outputFile("encryption_output.txt");
    if (!outputFile) {
        std::cerr << "Error opening file for writing.\n";
        return 1;
    }

    hight.encrypt(left, right, numRounds, outputFile);
    outputFile.close();

    std::cout << "Encryption output saved to encryption_output.txt\n";
    return 0;
}