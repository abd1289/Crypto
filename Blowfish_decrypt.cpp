...
private:
    ...
    // P-array and S-boxes initial values
    ...
    void init() {
        // Initialize S-boxes
        S.resize(4, std::vector<uint32_t>(256, 0));
        for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 256; ++j) {
                S[i][j] = std::stoul(S_INIT[i][j], nullptr, 16);
            }
        }
    }
...
public:
    ...
    std::vector<uint8_t> decryptData(const std::vector<uint8_t>& data) {
        ...
        for (size_t i = 0; i < data.size(); i += 8) {
            ...
            decrypt(left, right);
            for (int j = 0; j < 4; ++j) {
                decryptedData.push_back((left >> (24 - j * 8)) & 0xFF);
                decryptedData.push_back((right >> (24 - j * 8)) & 0xFF);
            }
        }
...
private:
    uint32_t F(uint32_t x) {
        ...
        uint8_t c = (x >> 8) & 0xFF;
        uint8_t d = x & 0xFF;
        uint32_t res = (S[0][a] + S[1][b]) ^ S[2][c];
        res += S[3][d];
        return res;
    }

    const std::vector<std::vector<std::string>> S_INIT = {
        {
...
int main() {
    std::ifstream encryptedFile("output_blowfish_encrypt.txt", std::ios::binary);
    std::ofstream decryptedFile("output_blowfish_decrypt.txt", std::ios::binary);
    ...
    std::vector<uint8_t> encryptedData(std::istreambuf_iterator<char>(encryptedFile), {});

    Blowfish blowfish;
    std::vector<uint8_t> decryptedData = blowfish.decryptData(encryptedData);

    decryptedFile.write(reinterpret_cast<const char*>(decryptedData.data()), decryptedData.size());
    decryptedFile.close();

    std::cout << "Decrypted data:\n" << std::string(decryptedData.begin(), decryptedData.end()) << std::endl;
