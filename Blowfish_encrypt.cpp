...
int main() {
    ...
    // Read input file
    std::string plaintext = readFile("input.txt");
    if (plaintext.empty()) {
    ...
    std::vector<uint8_t> ciphertextBytes = blowfish.encryptData(plaintextBytes);

    // Convert ciphertext bytes to hex string for display and saving
    std::stringstream ciphertextHex;
    for (uint8_t byte : ciphertextBytes) {
        ciphertextHex << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(byte);
    }
    std::string ciphertextHexStr = ciphertextHex.str();

    // Display ciphertext
    std::cout << "Ciphertext:" << std::endl;
    std::cout << ciphertextHexStr << std::endl;

    // Save ciphertext to file
    writeFile("output_blowfish_encrypt.txt", ciphertextHexStr);
