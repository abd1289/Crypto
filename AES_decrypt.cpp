...
void inv_add_round_keys(unsigned char* state, unsigned char* round_key) {
    for(int i = 0; i < 16; i++) {
        state[i] ^= round_key[i];
    }
}
...
int main() {
    ifstream inputFile("output_aes_encrypt.txt");
    if (!inputFile) {
    ...
    unsigned char* encryptedBytes = new unsigned char[len / 2];
    for (int i = 0; i < len; i += 2) {
        string byteString = encryptedMessage.substr(i, 2);
        encryptedBytes[i / 2] = stoi(byteString, nullptr, 16);
    }
    ...
    AES_decrypt(encryptedBytes, key);
